...
def main():
    ...
    def valid(model,data,criterion):
        model.eval()
        with torch.no_grad():   # do not record the gradient information
            y_pred = model(data)
    ...
    np.save('result/GAN_no_noise.npy',loss_train)   # save as .npy for further comparision
    print('GAN_no_noise——MSE of validation：',loss_val[-1])
    plt.show()
